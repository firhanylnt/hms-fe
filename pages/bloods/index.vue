...
export default {
    ...
    methods: {
        async get_data(){
            try {
                const url = `${process.env.apiBaseUrl}/bloods`
        ...
        create(){
            this.$router.push(`/bloods/create`)
        },

        move(id) {
            this.$router.push(`/bloods/${id}/edit`)
        },
        ...
        confirm(id) {
            Swal.fire({
                ...
                if (result.value) {
                    const url = `${process.env.apiBaseUrl}/bloods/delete/${id}`
                    await this.$axios.$post(url)
                    .then(() => {
                        Swal.fire("Deleted!", "bloods has been deleted.", "success");
                        this.get_data()
                    })
                }
            });
