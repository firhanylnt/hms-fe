...
export default {
    ...
    methods: {
        ...
        create(){
            this.$router.push(`/nurses/create`)
        },

        move(id) {
            this.$router.push(`/nurses/${id}/edit`)
        },
        ...
        confirm(id) {
            Swal.fire({
                ...
                if (result.value) {
                    const url = `${process.env.apiBaseUrl}/nurses/delete/${id}`
                    await this.$axios.$post(url)
                    .then(() => {
                        Swal.fire("Deleted!", "Nurses has been deleted.", "success");
                        this.get_data()
                    })
                }
            });
