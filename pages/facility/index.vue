  ...
  methods: {
    async get_data() {
      try {
        ...
        let url = `${process.env.apiBaseUrl}/facilities`;
    ...
    create() {
      this.$router.push(`/facility/create`);
    },

    move(id) {
      this.$router.push(`/facility/${id}/edit`);
    },
    ...
    confirm(id) {
      Swal.fire({
        ...
        if (result.value) {
          const url = `${process.env.apiBaseUrl}/facilities/delete/${id}`;
          await this.$axios.$post(url).then(() => {
            Swal.fire("Deleted!", "facilities has been deleted.", "success");
            this.get_data();
          });
        }
      });
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-sm-12 col-md-12">
                <div
                  v-if="userRole === 'Cashier' || userRole === 'Super Admin'"
                >
