...
export default {
  ...
  methods: {
    ...
    create() {
      this.$router.push(`/medicine/category/create`);
    },

    move(id) {
      this.$router.push(`/medicine/category/${id}/edit`);
    },
    ...
    confirm(id) {
      Swal.fire({
        ...
        if (result.value) {
          const url = `${process.env.apiBaseUrl}/medicine-categories/delete/${id}`;
          await this.$axios.$post(url).then(() => {
            Swal.fire(
              "Deleted!",
              "Medicine Category has been deleted.",
              "success"
            );
            this.get_data();
          });
