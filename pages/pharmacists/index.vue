...
export default {
    ...
    methods: {
        async get_data(){
            try {
                const url = `${process.env.apiBaseUrl}/pharmacists`
        ...
        create(){
            this.$router.push(`/pharmacists/create`)
        },

        move(id) {
            this.$router.push(`/pharmacists/${id}/edit`)
        },
        ...
        confirm(id) {
            Swal.fire({
                ...
                if (result.value) {
                    const url = `${process.env.apiBaseUrl}/pharmacists/delete/${id}`
                    await this.$axios.$post(url)
                    .then(() => {
                        Swal.fire("Deleted!", "Pharmacist has been deleted.", "success");
                        this.get_data()
                    })
                }
            });
